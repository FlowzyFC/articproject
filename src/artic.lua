...
function Library:change_theme(toTheme)
	...
	Library.DisplayName.Text = "Welcome, <font color='rgb(" ..  math.floor(c.R*255) .. "," .. math.floor(c.G*255) .. "," .. math.floor(c.B*255) .. ")'> <b>""</b> </font>"
...
function Library:create(options)
	...
	if readfile and writefile and isfile then
		if not isfile("articprojectSettings.json") then
			writefile("articprojectSettings.json", HTTPService:JSONEncode(settings))
		end
		settings = HTTPService:JSONDecode(readfile("articprojectSettings.json"))
	...
	options = self:set_defaults({
		...
		Link = "https://github.com/deeeity/articproject-lib"
	}, options)

	if getgenv and getgenv().articprojectUI then
		getgenv():articprojectUI()
		getgenv().articprojectUI = nil
	end
	...
	if getgenv then
		getgenv().articprojectUI = closeUI
	end
	...
	local displayName; do
		...
		local displayName = profile:object("TextLabel", {
			RichText = true,
			Text = "Welcome To Artic Project <font color='rgb(" ..  math.floor(c.R*255) .. "," .. math.floor(c.G*255) .. "," .. math.floor(c.B*255) .. ")'> <b>""</b> </font>",
			TextScaled = true,
	...
	creditsTab:credit{Name = "Flowzy", Description = "UI Library Developer & Script Dev", Discord = "Flowzy#3142", V3rmillion = "AbstractPoo"}
	creditsTab:credit{Name = "Requiem", Description = "UI Library Developer & Script Dev", Discord = "Requiem#2018", V3rmillion = "0xDEITY"}
